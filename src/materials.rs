...
pub trait Material: Sync {
    ...
    fn emitted(&self, r_in: &Ray, rec: &Hit, u: f64, v: f64, p: Vec3) -> SampledSpectrum; //Vec3 { return Vec3::ZEROS; }
...
pub struct Lambertian { pub albedo: Vec3, pub emit: SampledSpectrum }
impl Material for Lambertian {
    ...
    fn emitted(&self, r_in: &Ray, rec: &Hit, u: f64, v: f64, p: Vec3) -> SampledSpectrum {
        if rec.normal.dot(r_in.direction) < 0.0 { 
            return self.emit.clone(); 
        }
        return crate::spectrum::zero_spectrum();
    }
}
